...
commander_1.default
    ...
    .parse(process.argv);
var cli = function (internalArgs) { return __awaiter(void 0, void 0, void 0, function () {
    var provider, reset, credentials, loginAnswers, err, e_1, repoName, res, e_2, e_3;
    var _a;
    var _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            ...
            case 5:
                ...
                return [3 /*break*/, 7];
            case 6:
                err = new Error("Unknown Error with provider " + provider + " and credentials " + credentials);
                err.name = 'Unknown';
                throw err;
            ...
            case 8:
                e_1 = _c.sent();
                if (e_1.name === 'Unknown') {
                    console.error(chalk_1.default.red(e_1));
                    cli({ reset: true, provider: provider });
                    return [2 /*return*/];
                }
                else if (e_1.response.status === 401) {
                    console.error(chalk_1.default.red('wrong login credentials\n'));
                    cli({ reset: true, provider: provider });
                    return [2 /*return*/];
                }
                else {
                    console.error(chalk_1.default.red("something went wrong with status " + e_1.response.status + ", try again\nmessage: " + e_1.response.data.error + "\n"));
                    cli({ reset: true, provider: provider });
                    return [2 /*return*/];
                }
                return [3 /*break*/, 9];
            ...
            case 14:
                ...
                if (e_2.response.status === 401) {
                ...
                else if (e_2.response.status === 400) {
                    ...
                    console.error(chalk_1.default.red("something went wrong with status " + e_2.response.status + ", try again\nmessage: " + e_2.response.data.error + "\n"));
                    cli({ reset: true, provider: provider });
                    return [2 /*return*/];
                }
                return [3 /*break*/, 15];
            ...
            case 19:
                e_3 = _c.sent();
                if (e_3.response.status === 401) {
                    console.error(chalk_1.default.red('wrong login credentials\n'));
                    cli({ reset: true, provider: provider });
                    return [2 /*return*/];
                }
                else if (e_3.response.status === 422) {
                    console.error(chalk_1.default.red("You already have a repo called " + repoName + "\nTry a different repo name.\n"));
                    cli({ reset: false, provider: provider });
                    return [2 /*return*/];
                }
                else if (e_3.response.status === 403) {
                    console.error(chalk_1.default.red("Request limit exceeded.  Try again in one hour.\n"));
                    return [2 /*return*/];
                }
                else {
                    console.error(chalk_1.default.red("something went wrong with status " + e_3.response.status + ", try again\nmessage: " + e_3.response.message + "\n"));
                    cli({ reset: true, provider: provider });
                    return [2 /*return*/];
                }
                return [3 /*break*/, 20];
